...
def postprocessed_from_raw(rawImgFname, verbose=False):

    if rawImgFname.lower().endswith('.nef'):  # If this is a raw image

        # Extract rawImage
        rawImage = rawpy.imread(rawImgFname)
...
def bayer4up_from_raw(rawImgFname, verbose=False):
...
def zScore4up_from_raw(rawImgFname, stDev, verbose=False):
...
def ZscoreExceedsZlimit4up_from_raw(rawImgFname, stDev, zLimit, verbose=False):
...
def prep_img_for_display(imgIn, maxDisplayWidth=1024, maxDisplayHeight=768, verbose=False):
    ...
    # imgMaxDim = max(imgOut.height, imgOut.width)  # Get image H & W
    #
    # if verbose:
    #     print('imgMaxDim = {}'.format(imgMaxDim))
    #
    #     imgresizeration switch to new variables
    #
    # imgResizeLabel = ''  # initialize to blank in case no resizing is necessary
    # if imgMaxDim > maxDisplaySize:
    #     shrinkN = int(
    #         np.ceil(np.log2(imgMaxDim / maxDisplaySize)))  # 2^n power by which image will be resized by shrink()
    #     imgResizeLabel = 'Resized from {}x{} by factor of {}    (2^{})'.format(imgOut.width, imgOut.height, 2 ** shrinkN,
    #                                                                            shrinkN)
    #     imgOut.shrink(shrinkN)  # shrink by factor of 2^shrinkN
    #
    # # Set the x,y location of img to correct for width
    # imgOut.x, imgOut.y = int((maxDisplaySize - imgOut.width) / 2), int((maxDisplaySize - imgOut.height) / 2)
